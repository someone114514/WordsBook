...
import { importWordList, previewWordList } from '../modules/wordbook/wordListImportService'
...
async function makePreview() { busy.value = true; try { preview.value = await previewWordList(importText.value); importStep.value = 2 } finally { busy.value = false } }
async function runImport() { busy.value = true; try { report.value = await importWordList(listId.value, importText.value); importStep.value = 3; await load() } finally { busy.value = false } }
...
<template>
  <main v-if="current" class="page-shell list-detail">
    <header class="detail-header"><RouterLink class="btn" to="/lists">返回词表</RouterLink><div><h1>{{ current.name }}</h1><p>{{ current.wordCount }} 个单词 · {{ current.studyEnabled ? '参与每日学习' : '不参与每日学习' }}</p></div></header>
    ...
    <section v-if="tab === 'words'" class="panel">
      ...
      <div v-if="filtered.length" class="word-list-clean"><label v-for="row in filtered" :key="row.item.wordId"><input v-model="selected" type="checkbox" :value="row.item.wordId"/><span><strong>{{ row.entry?.headword ?? row.item.entryId }}</strong><small>{{ row.item.note || '无备注' }}</small></span></label></div>
    ...
    <section v-else-if="tab === 'import'" class="panel import-flow">
      ...
      <template v-if="importStep === 1"><p>粘贴内容，或选择 TXT / CSV / TSV。支持 word、meaning、note、tags 列。</p><input type="file" accept=".txt,.csv,.tsv,text/plain,text/csv" @change="onFile"/><textarea v-model="importText" class="notes-area import-area" rows="10" placeholder="word,meaning,note,tags&#10;resilient,有韧性的,重点词,考试"></textarea><button class="btn btn-primary" :disabled="busy || !importText.trim()" @click="makePreview">预览导入</button></template>
      <template v-else-if="importStep === 2 && preview"><div class="preview-stats"><div><strong>{{ preview.rows.length }}</strong><span>总行数</span></div><div><strong>{{ preview.matched }}</strong><span>词典匹配</span></div><div><strong>{{ preview.pending }}</strong><span>待补全</span></div><div><strong>{{ preview.duplicates }}</strong><span>重复</span></div><div><strong>{{ preview.invalid }}</strong><span>无效</span></div></div><button class="btn" @click="importStep=1">返回修改</button><button class="btn btn-primary" :disabled="busy" @click="runImport">确认导入</button></template>
      <template v-else-if="report"><h2>导入完成</h2><p>匹配 {{ report.matched }}，新建 {{ report.created }}，待补全 {{ report.pending }}，重复 {{ report.duplicates }}，无效 {{ report.invalid }}。</p><button class="btn btn-primary" @click="tab='words'; importStep=1; importText=''">查看单词</button></template>
    </section>
