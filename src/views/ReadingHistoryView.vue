...
function canResume(session: ReadingSession): boolean {
  return session.dayKey === dayjs().format('YYYY-MM-DD')
}
...
<template>
  <main class="page-shell reading-history-page">
    <header class="detail-header"><RouterLink class="btn" to="/review">返回学习</RouterLink><div><h1>文章记录</h1><p>保留已经生成的正文、题目进度和翻译</p></div></header>
    <section v-if="sessions.length" class="reading-history-list">
      <article v-for="item in sessions" :key="item.sessionId" class="panel reading-history-card">
        <div class="reading-history-heading"><div><strong>{{ item.title || '语境文章' }}</strong><p>{{ item.dayKey }} · {{ item.targetWordIds.length }} 个目标词 · {{ statusLabel(item) }}</p></div><div class="actions"><RouterLink v-if="canResume(item)" class="btn btn-primary" :to="{ path: '/review/reading', query: { session: `daily:${item.dayKey}`, batch: item.batchIndex } }">继续阅读</RouterLink><button class="btn" type="button" :aria-expanded="expandedId === item.sessionId" @click="expandedId = expandedId === item.sessionId ? '' : item.sessionId">{{ expandedId === item.sessionId ? '收起' : '查看' }}</button></div></div>
